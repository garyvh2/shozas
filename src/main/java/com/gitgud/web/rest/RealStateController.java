...
public class RealStateController extends ApiBaseController {
    ...
    public ApiResultModel<ApiSearchResults> searchAllTypes(@RequestBody ApiSearchParams apiSearchParams) throws Exception {
        return GetApiResultModel(() ->{
            ApiSearchResults results = new ApiSearchResults();
            results.setElements(realStateService.getRealStateElements(ResultType.All, apiSearchParams, results).stream().map(l -> RealStateUtils.toApiRealState(l)).collect(Collectors.toList()));
            return results;
        } );
    }
